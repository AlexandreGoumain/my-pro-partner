...
export default function ReparationsPage() {
  ...
  const { data: reparationsData, isLoading: isLoadingReparations } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (priorite && priorite !== "all") params.set("priorite", priorite);
      if (search) params.set("search", search);

      const response = await fetch(`/api/reparations?${params.toString()}`);
      if (!response.ok) throw new Error("Failed to fetch reparations");
        ...
        {isLoadingReparations ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {reparations.map((reparation: any) => (
              <RepairCard key={reparation.id} reparation={reparation} />
            ))}
          </div>
        )}
      </div>
