            ...
            <div className="min-h-screen bg-black/2 flex items-center justify-center p-4">
                <Card className="max-w-lg w-full p-12 border-black/8 shadow-sm">
                    <div className="flex flex-col items-center space-y-4">
                        <Loader2 className="w-8 h-8 text-black/40 animate-spin" strokeWidth={2} />
                        <p className="text-[14px] text-black/60">Chargement de la facture...</p>
                    </div>
                </Card>
            </div>
        );
            ...
            <div className="min-h-screen bg-black/2 flex items-center justify-center p-4">
                <Card className="max-w-lg w-full p-8 border-black/8 shadow-sm">
                    <div className="flex flex-col items-center text-center space-y-4">
                        <FileText className="w-12 h-12 text-red-500" strokeWidth={2} />
                        <h1 className="text-[20px] font-semibold tracking-[-0.01em] text-black">
                            {error || "Facture non trouvée"}
                        </h1>
                        <p className="text-[14px] text-black/60">
                            Veuillez vérifier le lien ou contacter l'entreprise.
                        </p>
                    </div>
                </Card>
            </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-black/5 rounded-lg p-6 space-y-4">
                        <div className="flex justify-between items-center">
                            <span className="text-[13px] text-black/60">Facture</span>
                            <span className="text-[15px] font-semibold text-black">{invoice.numero}</span>
                        </div>
                        <div className="flex justify-between items-center">
                            <span className="text-[13px] text-black/60">Client</span>
                            <span className="text-[14px] font-medium text-black">{clientName}</span>
                        </div>
                        <div className="flex justify-between items-center">
                            <span className="text-[13px] text-black/60">Date d'émission</span>
                            <span className="text-[14px] font-medium text-black">
                                {format(new Date(invoice.dateEmission), "dd MMMM yyyy", { locale: fr })}
                            </span>
                        </div>
                        <div className="h-px bg-black/10 my-4" />
                        <div className="flex justify-between items-center">
                            <span className="text-[15px] font-semibold text-black">Montant à payer</span>
