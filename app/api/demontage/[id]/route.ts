...
export async function GET(
  ...
  return withErrorHandling(async () => {
    ...
    const { resource: demontage } = await verifyResourceAccess(
      id,
      (id) => prisma.demontageArticle.findUnique({
        where: { id },
        include: {
          articleSource: {
            include: {
...
export async function DELETE(
  ...
  return withErrorHandling(async () => {
    ...
    const pieceIds = demontage.articleSource.piecesDetachees.map((piece) => piece.id);
    ...
    if (pieceIds.length > 0) {
      ...
      if (usedPieces) {
        return NextResponse.json(
          {
            message: "Impossible de supprimer ce démontage : des pièces ont été utilisées dans des documents (devis, factures, etc.)",
          },
          { status: 400 }
        );
      }
