...
export async function GET(_req: NextRequest) {
  return withErrorHandling(async () => {
    ...
    // By status
    const byStatus = reparations.reduce((acc, r) => {
      acc[r.statut] = (acc[r.statut] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    // By priority
    const byPriority = reparations.reduce((acc, r) => {
    ...
    const enCours = reparations.filter((r) =>
      ["DEPOSE", "DIAGNOSTIC", "DEVIS_ENVOYE", "ATTENTE_PIECES", "EN_COURS"].includes(r.statut)
    ).length;

    // Ready for pickup
    const pretes = reparations.filter((r) => r.statut === "PRETE").length;
    ...
    // By technician
    const byTechnicien = reparations.reduce((acc, r) => {
      if (r.technicienId) {
    ...
    const totalHeuresTechnicien = reparations.reduce((sum, r) => {
