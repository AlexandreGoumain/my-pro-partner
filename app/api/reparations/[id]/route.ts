  ...
  return withErrorHandling(async () => {
    ...
    const updatedReparation = await prisma.reparation.update({
      ...
      data: {
        ...
        marque: data.marque !== undefined ? data.marque || null : reparation.marque,
        modele: data.modele !== undefined ? data.modele || null : reparation.modele,
        numeroSerie: data.numeroSerie !== undefined ? data.numeroSerie || null : reparation.numeroSerie,
        motAuthentification: data.motAuthentification !== undefined ? data.motAuthentification || null : reparation.motAuthentification,
        panne: data.panne || reparation.panne,
        etatVisuel: data.etatVisuel !== undefined ? data.etatVisuel || null : reparation.etatVisuel,
        accessoires: data.accessoires !== undefined ? data.accessoires || null : reparation.accessoires,
        priorite: data.priorite || reparation.priorite,
        dateEstimeeRetour: data.dateEstimeeRetour || reparation.dateEstimeeRetour,
        notesInternes: data.notesInternes !== undefined ? data.notesInternes || null : reparation.notesInternes,
        notesTechnicien: data.notesTechnicien !== undefined ? data.notesTechnicien || null : reparation.notesTechnicien,
      },
      include: {
        client: true,
        technicien: {
...
export async function DELETE(
  ...
  return withErrorHandling(async () => {
    ...
    if (reparation.documentId) {
      return NextResponse.json(
        { message: "Impossible de supprimer : cette réparation est liée à un document" },
        { status: 400 }
      );
    }
