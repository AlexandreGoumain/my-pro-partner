...
export async function POST(
    ...
    return withErrorHandling(
        async () => {
            ...
            const { diagnosticDetail, devisEstime, delaiReparation } =
                result.data;

            // Calculate estimated return date
            const dateEstimeeRetour = delaiReparation
                ? ReparationService.calculateEstimatedReturn(
                      reparation.dateDepot,
                      delaiReparation
                  )
                : null;
            ...
            const updatedReparation = await prisma.reparation.update({
                ...
                data: {
                    ...
                    devisEstime,
                    delaiReparation:
                        delaiReparation || reparation.delaiReparation,
                    dateEstimeeRetour:
                        dateEstimeeRetour || reparation.dateEstimeeRetour,
                    // Automatically move to diagnostic status if not already
                    statut:
                        reparation.statut === "DEPOSE"
                            ? "DIAGNOSTIC"
                            : reparation.statut,
                },
                include: {
