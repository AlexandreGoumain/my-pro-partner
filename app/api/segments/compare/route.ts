    ...
    return withApiHandler(
        async (ctx) => {
            ...
            if (!Array.isArray(segmentIds) || segmentIds.length < 2) {
                throw new ValidationError("Au moins 2 segments sont requis pour la comparaison");
            }

            if (segmentIds.length > 5) {
                throw new ValidationError("Maximum 5 segments peuvent être comparés");
            }
            ...
            if (segments.length !== segmentIds.length) {
                throw new NotFoundError("Un ou plusieurs segments n'ont pas été trouvés");
            }
        ...
        {
            context: { resourceName: "Segment", operation: "compare" },
        }
    );
}
