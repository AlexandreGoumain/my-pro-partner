...
// GET /api/gestion-locative/baux - List leases with filters
export async function GET(request: NextRequest) {
    try {
        ...
        return NextResponse.json(
            { error: "Failed to fetch baux" },
            { status: 500 }
        );
    }
}

// POST /api/gestion-locative/baux - Create new lease
export async function POST(request: NextRequest) {
    try {
        ...
        if (!body.bienId || !body.locataireId || !body.proprietaireId) {
            return NextResponse.json(
                { error: "Bien, locataire et propriétaire requis" },
                { status: 400 }
            );
        }
