    ...
    const handleImport = useCallback(async () => {
        ...
        try {
            ...
            if (
                ...
                toast.success(
                    `${transformedData.length} ligne(s) importée(s) avec succès`
                );
            }
            ...
            if (
                ...
                message.includes("existent déjà")
            ) {
                    ...
                    parsedData.forEach((row, rowIndex) => {
                        ...
                        if (
                            ...
                            errors.push({
                                ...
                                field: "Email",
                                message:
                                    "Cet email existe déjà dans votre base de données",
                            });
                        }
                    });
                }
            ...
            if (
                message.includes("emails en double") &&
                message.includes("dans l'import")
            ) {
            ...
            if (
                ...
                if (phoneMatch) {
                    const duplicatePhones = phoneMatch[1]
                        .split(", ")
                        .map((p) => p.split(" (")[0]);
                    ...
                    parsedData.forEach((row, rowIndex) => {
                        ...
                        if (
                            phoneValue &&
                            duplicatePhones.includes(phoneValue)
                        ) {
                            errors.push({
                                row: rowIndex,
                                field: "Téléphone",
                                message:
                                    "Ce numéro existe déjà dans votre base de données",
                            });
                        }
                    });
                }
            ...
            if (
                message.includes("numéros de téléphone en double") &&
                message.includes("dans l'import")
            ) {
            ...
            if (errors.length > 0) {
                ...
                toast.error("Erreur lors de l'import", {
                    description:
                        "Veuillez corriger les erreurs en rouge dans le tableau",
                });
            } else {
                toast.error(message);
            }
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <ul className="text-[13px] text-black/70 space-y-2">
                                            <li className="flex items-start gap-2">
                                                ...
                                                <span>
                                                    <span className="font-medium text-black/80">Colonnes requises:</span>{" "}
                                                    {mappings
                                                        .filter((m) => m.required)
                                                        .map((m) => m.label)
                                                        .join(", ")}
                                            ...
                                            <li className="flex items-start gap-2">
                                                <span className="text-black/40 mt-0.5">•</span>
                                                <span>
                                                    <span className="font-medium text-black/80">Colonnes optionnelles:</span>{" "}
                                                    {mappings
                                                        .filter((m) => !m.required)
                                                        .map((m) => m.label)
                                                        .join(", ")}
                                            ...
                                            <li className="flex items-start gap-2">
                                                <span className="text-black/40 mt-0.5">•</span>
                                                <span><span className="font-medium text-black/80">Encodage:</span> UTF-8</span>
                                            </li>
                                            <li className="flex items-start gap-2">
                                                <span className="text-black/40 mt-0.5">•</span>
                                                <span><span className="font-medium text-black/80">Séparateur:</span> virgule (,)</span>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                    ...
                                    <TableBody>
                                        {parsedData.map((row, rowIndex) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <TableCell
                                                        className={`font-mono text-[12px] text-black/40 sticky left-0 z-10 ${hasError ? "bg-red-50 border-l-4 border-l-red-500" : "bg-white"}`}
                                                    >
                                                        {rowIndex + 1}
                                                    </TableCell>
                                                    {mappings.map((mapping) => {
                                                    ...
                                                    <TableCell
                                                        className={`sticky right-0 z-10 ${hasError ? "bg-red-50" : "bg-white"}`}
                                                    >
                                                        <Button
                                                            variant="ghost"
                                                            size="icon"
