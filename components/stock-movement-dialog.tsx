...
export function StockMovementDialog({
  ...
  }, [open, form, defaultArticleId]);

  // Filtrer les articles avec gestion de stock activée
  const stockEnabledArticles = articlesData?.filter(
    (article) => article.gestionStock === true
  ) || [];
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select
                    ...
                    value={field.value}
                    disabled={loadingArticles || !!defaultArticleId}
                  >
                    <FormControl>
                      <SelectTrigger>
                    ...
                    <SelectContent>
                      {stockEnabledArticles.map((article) => (
                        <SelectItem key={article.id} value={article.id}>
                          {article.reference} - {article.nom} (Stock:{" "}
                          {article.stock})
                        </SelectItem>
                      ))}
                    </SelectContent>
                  ...
                  {selectedArticle && (
                    <FormDescription>
                      Stock actuel: {selectedArticle.stock} | Stock min:{" "}
