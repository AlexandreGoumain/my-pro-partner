...
export class PaymentLinkService {
     ...
     */
    static isPaymentLinkValid(paymentLink: {
        actif: boolean;
        ...
        // Vérifier expiration
        if (
            paymentLink.dateExpiration &&
            new Date() > paymentLink.dateExpiration
        ) {
            return false;
        }
    ...
    static async createCheckoutSession({
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            metadata: {
                ...
                paymentLink.metadata !== null &&
                !Array.isArray(paymentLink.metadata)
                    ? (paymentLink.metadata as Record<string, unknown>)
                    : {}),
            },
        });
    ...
    static async getPaymentLinks(entrepriseId: string) {
        ...
        return paymentLinks.map((link) => ({
            ...
            tauxConversion:
                link.nombreVues > 0
                    ? ((link.nombrePaiements / link.nombreVues) * 100).toFixed(
                          1
                      )
                    : "0",
        }));
