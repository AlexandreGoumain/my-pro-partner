...
export function useDocumentForm({
        ...
        if (existingDocument && documentId) {
            ...
            setFormData({
                ...
                dateEcheance: existingDocument.dateEcheance
                    ? new Date(existingDocument.dateEcheance)
                          .toISOString()
                          .split("T")[0]
                    : "",
                validite_jours:
                    existingDocument.validite_jours ||
                    DOCUMENT_DEFAULTS.VALIDITY_DAYS,
                notes: existingDocument.notes || "",
                conditions_paiement: existingDocument.conditions_paiement || "",
            });

            // Préremplir les lignes
            const loadedLines: LineItem[] = existingDocument.lignes.map(
                (ligne) => ({
                    id: ligne.id,
    ...
    const totals = useMemo(() => {
        const totalHT = lines.reduce(
            (sum, line) => sum + Number(line.montant_ht || 0),
            0
        );
        const totalTVA = lines.reduce(
            (sum, line) => sum + Number(line.montant_tva || 0),
            0
        );
        const totalTTC = lines.reduce(
            (sum, line) => sum + Number(line.montant_ttc || 0),
            0
        );
    ...
    const validateForm = (): boolean => {
        ...
        if (emptyLines.length > 0) {
            toast.error(
                "Veuillez remplir la désignation pour toutes les lignes"
            );
            return false;
        }
        ...
        const validLines = lines.filter(
            (line) => Number(line.montant_ttc || 0) > 0
        );
        if (validLines.length === 0) {
            toast.error(
                "Au moins une ligne doit avoir un montant supérieur à 0"
            );
            return false;
        }
    ...
    const handleSubmit = async (statut: DocumentStatus) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    error.message || MESSAGES.ERRORS.CREATE_DOCUMENT
                );
            }
